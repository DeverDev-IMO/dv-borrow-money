...
class Report extends Connect
{
  ...
  public function listreportDebtorType1($table, $date_start, $date_end)
  {
    ...
    ";
    if ($_POST['linework_id'] != '') {
      // $sql .= " WHERE (contract_linework_id=:linework_id) AND (contract_status_cancel = 1) AND ((cash_number_installment-paysum_pay_installment)<10)";
      $sql .= " WHERE (contract_linework_id=:linework_id) AND (contract_status_cancel = 1) AND (cash_date_end >= :date_start) AND (contract_status_closebalance=0)";
      $sql .= " ORDER BY contract_id ASC";
      $statement = $pdo->prepare($sql);
      $statement->bindValue(":date_start", $date_start);
      // $statement->bindValue(":date_end", $date_end);
      ...
      // $sql .= " WHERE (contract_status_cancel = 1) AND ((cash_number_installment-paysum_pay_installment)<10)";
      $sql .= " WHERE (contract_status_cancel = 1) AND (cash_date_end >= :date_start) AND (contract_status_closebalance=0)";
      $sql .= " ORDER BY contract_id ASC";
      $statement = $pdo->prepare($sql);
      $statement->bindValue(":date_start", $date_start);
      // $statement->bindValue(":date_end", $date_end);
  ...
  public function listreportDebtorType2($table, $date_start, $date_end)
  {
    $pdo = parent::getInstance();
    $sql = "SELECT * FROM $table LEFT JOIN customer ON contract_customer_id=cus_id 
    LEFT JOIN cashloan ON contract_cashloan_id=cash_id 
    ...
    ";
    if ($_POST['linework_id'] != '') {
      // $sql .= " WHERE (contract_linework_id=:linework_id) AND (contract_status_cancel = 1) AND ((cash_number_installment-paysum_pay_installment)>10) AND (cash_date_end >= :date_start)";
      // $sql .= " WHERE (contract_linework_id=:linework_id) AND (contract_status_cancel = 1) AND ((cash_number_installment-paysum_pay_installment)>10) AND (cash_date_end >= :date_start)";
      $sql .= " WHERE (contract_linework_id=:linework_id) AND (contract_status_cancel = 1) AND (cash_date_end >= :date_start) AND (contract_status_closebalance=0)";
      $sql .= " ORDER BY contract_id ASC";
      $statement = $pdo->prepare($sql);
      ...
      // $sql .= " WHERE (contract_status_cancel = 1) AND ((cash_number_installment-paysum_pay_installment)>10) AND (cash_date_end <= :date_start)";
      // $sql .= " WHERE (contract_status_cancel = 1) AND ((cash_number_installment-paysum_pay_installment)>10) AND (cash_date_end >= :date_start)";
      $sql .= " WHERE (contract_status_cancel = 1) AND (cash_date_end >= :date_start) AND (contract_status_closebalance=0)";
      $sql .= " ORDER BY contract_id ASC";
      $statement = $pdo->prepare($sql);
  ...
  public function listreportDebtorType3($table, $date_start, $date_end) //ลูกหนี้หมดสัญญา
  {
    ...
    if ($_POST['linework_id'] != '') {
      ...
      $sql .= " WHERE (cash_date_end <= :date_start) AND (contract_linework_id=:linework_id) AND (contract_status_cancel = 1) AND (contract_status_closebalance=0)";
      $sql .= " ORDER BY contract_id ASC";
      $statement = $pdo->prepare($sql);
      $statement->bindValue(":date_start", $date_start);
      // $statement->bindValue(":date_end", $date_end);
      ...
      $sql .= " WHERE (cash_date_end <= :date_start) AND (contract_status_cancel = 1) AND (contract_status_closebalance=0)";
      $sql .= " ORDER BY contract_id ASC";
      $statement = $pdo->prepare($sql);
      $statement->bindValue(":date_start", $date_start);
      // $statement->bindValue(":date_end", $date_end);
  ...
  public function listreportDebtorType4($table, $date_start, $date_end) //หนี้ NPL [ หลังจากหมดสัญญา 30 วัน ]
  {
    $pdo = parent::getInstance();
    $sql = "SELECT * FROM $table LEFT JOIN customer ON contract_customer_id=cus_id 
    LEFT JOIN marry_detail ON contract_marrydetail_id=marryd_id 
  ...
  public function listreportDebtorType5($table, $date_start, $date_end) //ลูกหนี้การค้ารวม ตั้งแต่เปิดมายังไม่ปิดบัญชี
  {
    $pdo = parent::getInstance();
    $sql = "SELECT * FROM $table LEFT JOIN customer ON contract_customer_id=cus_id 
    LEFT JOIN cashloan ON contract_cashloan_id=cash_id 
    ...
    ";
    if ($_POST['linework_id'] != '') {
      $sql .= " WHERE (contract_linework_id=:linework_id) AND (contract_status_cancel = 1) AND (contract_status_closebalance=0)";
      $sql .= " ORDER BY contract_id ASC";
      $statement = $pdo->prepare($sql);
      // $statement->bindValue(":date_start", $date_start);
      // $statement->bindValue(":date_end", $date_end);
      ...
      $sql .= " WHERE (contract_status_cancel = 1) AND (contract_status_closebalance=0)";
      $sql .= " ORDER BY contract_id ASC";
      $statement = $pdo->prepare($sql);
      // $statement->bindValue(":date_start", $date_start);
      // $statement->bindValue(":date_end", $date_end);
  ...
  public function listreportDebtorType6($table, $date_start, $date_end) //ลูกหนี้การค้าไม่รวม NPL
  {
    $pdo = parent::getInstance();
    $sql = "SELECT * FROM $table LEFT JOIN customer ON contract_customer_id=cus_id 
    LEFT JOIN marry_detail ON contract_marrydetail_id=marryd_id 
