...
class Contract extends Connect
{
  ...
  public function deleteClient($table, $id)
  {
    ...
    // $statement6->execute();
    $sqlupdate = "UPDATE $table SET contract_status_cancel=:status_cancel WHERE contract_id=:id";
  ...
  public function cashloaninsertClient($table1, $data)
  {
    ...
    } elseif ($data['installment_limit'] == 73) {
      if ($numberinstallmentroundup > 73) {
        return 'dayover73';
        exit();
      }
    } else {
      if ($numberinstallmentroundup > 76) {
        return 'dayover76';
        exit();
      }
    }
    ...
    $sqllastconnum = "SELECT * FROM contract LEFT JOIN cashloan ON contract_cashloan_id=cash_id WHERE contract_linework_id = :id ORDER BY contract_id DESC limit 0,1";
    ...
    list($datestartYear, $datestartMomth, $datestartDay) = explode("-", $data['cash_date_start']); //ข้อมูลวันที่เริ่มผ่อนจ่าย แยกวันเดือนปี

    // $connumyearcurrent = date('Y') + 543;
    // $connummonthcurrent = date('m');
    $connumyearcurrent = $datestartYear + 543;
    $connummonthcurrent = $datestartMomth;
    ...
    // $ordercalsubstr = substr($datalastidconnum['contract_number'], 2, 10); //edit 15-9-66
    // list($oldYear, $oldMomth, $oldDay) = explode("-", $datalastidconnum['cash_date_start']); //ข้อมูลสัญญาแถวก่อน

    // $thaiYearold = $oldYear + 543;
    //////////////////////////////edit 8-9-66/////////////////////////////
    if ($countchk > 0) { //เช็คว่าสายงานนั้นมีเเถวก่อนหน้าไหม
  ...
  public function getdataCardid($table, $id)
  {
    ...
    $data1 = array();
    $pdo = parent::getInstance();
    // $sql = "SELECT * FROM $table WHERE cus_card_id LIKE :cardid";
    $sql = "SELECT * FROM $table WHERE cus_card_id=:cardid";
    $statement = $pdo->prepare($sql);
    ...
    $sql1 = "SELECT * FROM guarantor WHERE guarantor_card_id=:cardid";
    $statement1 = $pdo->prepare($sql1);
    $statement1->bindValue(":cardid", $id);
    $statement1->execute();
  ...
  public function getdatacustomerInfo($table)
  {
    $data = array();
    $data1 = array();
    $data2 = array();
    ...
    $sql = "SELECT * FROM contract LEFT JOIN customer ON contract_customer_id=cus_id 
    LEFT JOIN set_prename ON setpre_id=cus_prename
    LEFT JOIN set_statusmarry ON setmar_id=cus_statusmarry
    LEFT JOIN set_statusaddress ON setadd_id=cus_statusaddress
    LEFT JOIN set_cohabiting ON setcoh_id=cus_cohabiting
    ...
    LEFT JOIN amphures ON amphures_id=cus_district
    LEFT JOIN districts ON districts_id=cus_sub_district
    WHERE cus_card_id = :id ORDER BY cus_id DESC LIMIT 1";
    $statement = $pdo->prepare($sql);
    $statement->bindValue(":id", $_REQUEST['cardid']);
    $statement->execute();
    $result = $statement->fetchAll();
    $countchkresult = $statement->rowCount();
    foreach ($result as $row) {
      $data = $row;
    }
    ////////////////////////ข้อมูลซ้ำ 2 รอบ (สถานที่ทำงาน จังหวัด อำเภอ ตำบล รหัสไปรษณีย์)////////////////////////////////
    $sql1 = "SELECT * FROM contract LEFT JOIN customer ON contract_customer_id=cus_id 
    LEFT JOIN set_prename ON setpre_id=cus_prename
    LEFT JOIN set_statusmarry ON setmar_id=cus_statusmarry
    LEFT JOIN set_statusaddress ON setadd_id=cus_statusaddress
    LEFT JOIN set_cohabiting ON setcoh_id=cus_cohabiting
    ...
    WHERE cus_card_id = :id ORDER BY cus_id DESC LIMIT 1";
    $statement1 = $pdo->prepare($sql1);
    $statement1->bindValue(":id", $_REQUEST['cardid']);
    $statement1->execute();
    $result1 = $statement1->fetchAll();
    // $countchkresult = $statement1->rowCount();
    foreach ($result1 as $row1) {
      $data1 = $row1;
    }

    ////////////////////////ข้อมูลผู้ค้ำประกัน////////////////////////////////
    $sql2 = "SELECT * FROM contract LEFT JOIN customer ON contract_customer_id=cus_id 
    LEFT JOIN guarantor ON contract_guarantor_id=guarantor_id
    LEFT JOIN set_prename ON setpre_id=guarantor_prename
    LEFT JOIN set_statusmarry ON setmar_id=guarantor_statusmarry
    LEFT JOIN set_statusaddress ON setadd_id=guarantor_statusaddress
    ...
    LEFT JOIN amphures ON amphures_id=guarantor_district
    LEFT JOIN districts ON districts_id=guarantor_sub_district
    WHERE guarantor_card_id = :id ORDER BY guarantor_id DESC LIMIT 1";
    $statement2 = $pdo->prepare($sql2);
    $statement2->bindValue(":id", $_REQUEST['cardid']);
    $statement2->execute();
  ...
  public function getdataCardidGuarantor($table, $id)
  {
    $data = array();
    $data1 = array();
    $pdo = parent::getInstance();
    $sql = "SELECT * FROM guarantor WHERE guarantor_card_id=:cardid";
    $statement = $pdo->prepare($sql);
    $statement->bindValue(":cardid", $id);
    $statement->execute();
    $result = $statement->fetchAll();
    ...
    $sql1 = "SELECT * FROM customer WHERE cus_card_id=:cardid";
    $statement1 = $pdo->prepare($sql1);
    $statement1->bindValue(":cardid", $id);
    $statement1->execute();
    $result1 = $statement1->fetchAll();
  ...
  public function getdataguarantorInfo($table)
  {
    $data = array();
    $data1 = array();
    $data2 = array();
    ...
    ////////////////////////ข้อมูลซ้ำ 2 รอบ ข้อมูลคู่สมรส (สถานที่ทำงาน จังหวัด อำเภอ ตำบล รหัสไปรษณีย์)////////////////////////////////
    $sql1 = "SELECT * FROM contract LEFT JOIN customer ON contract_customer_id=cus_id 
    LEFT JOIN guarantor ON contract_guarantor_id=guarantor_id
    LEFT JOIN set_prename ON setpre_id=guarantor_prename_marry
    LEFT JOIN set_statuswork ON setwork_id=guarantor_statuswork_marry
    LEFT JOIN provinces ON province_id=guarantor_province_marry
    LEFT JOIN amphures ON amphures_id=guarantor_district_marry
    LEFT JOIN districts ON districts_id=guarantor_sub_district_marry
    WHERE guarantor_card_id = :id ORDER BY guarantor_id DESC LIMIT 1";
  ...
  public function updatenumbercontractClient($table, $data, $id)
  {
    $pdo = parent::getInstance();
    $sql = "UPDATE $table SET  contract_number=:contract_number WHERE contract_id=:id";
    $statement = $pdo->prepare($sql);
    $statement->execute(array(
      'id' => $data['contract_id'],
  ...
  //ข้อมูลสัญญาซื้อ-ขายที่ยกเลิก
  public function listContractCancelClient($table)
  {
    $pdo = parent::getInstance();
    // if($_SESSION['sess_accessrights']=='superadmin'){
    $sql = "SELECT * FROM $table LEFT JOIN customer ON contract_customer_id=cus_id 
    ...
    LEFT JOIN cashloan ON contract_cashloan_id=cash_id 
    LEFT JOIN set_prename ON cus_prename=setpre_id 
    WHERE contract_status_cancel = 0
    ORDER BY contract_id DESC";
    $statement = $pdo->query($sql);
    $statement->execute();
    return $statement->fetchAll();
