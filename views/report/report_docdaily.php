...
$head = '
    ...
    ';
$headerreport = '<div style="border: 0px solid black;padding:0px;">
    <table style="width:100%;margin-top:5px;" cellpadding=0 cellspacing=0 border="0">
      ...
      <table style="margin-top:12px;width:100%;border-collapse: collapse;" cellpadding=3 cellspacing=1 border="1">
            <tr style="border: 1px solid black;">
                <td style="background-color: #D3D3D3;padding:5px;text-align:center;font-size:13px;width:45px;">ลำดับ</td>
                <td style="background-color: #D3D3D3;font-size:13px;width:95px;">เลขที่สัญญา</td>
                <td style="background-color: #D3D3D3;font-size:13px;width:150px;">ชื่อ - สกุล</td>
                <td style="background-color: #D3D3D3;font-size:13px;width:60px;">ชื่อเล่น</td>
                <td style="background-color: #D3D3D3;font-size:13px;width:110px;">เบอร์มือถือ</td>
                ...
                <td style="background-color: #D3D3D3;font-size:13px;width:70px;" align="right">ยอดค้าง</td>
                <td style="background-color: #D3D3D3;font-size:13px;" align="right">ชำระจริง</td>
            </tr>
        </table>
        </div>';
$content = '
<table style="width:100%;border-collapse: collapse;" cellpadding=3 cellspacing=1 border="1">';
...
foreach ($report->listreportdailyClient("contract", $date) as $client) :
    ...
    if ($client['contract_status_closebalance'] == 1) {
        if ($date <= $client['contract_date_closebalance']) {
            $content .= '
          <tr style="border: 1px solid black;">
            <td style="padding:5px;font-size:12px;text-align:center;width:45px">' . $i . '</td>
            <td style="padding:5px;font-size:12px;width:95px;">' . $client['contract_number'] . '</td>
            <td style="padding:5px;font-size:12px;width:150px;">' . $client['setpre_name'] . $client['cus_firstname'] . ' ' . $client['cus_lastname'] . '</td>
            <td style="padding:5px;font-size:12px;width:60px;">' . $client['cus_nickname'] . '</td>
            <td style="padding:5px;font-size:12px;width:110px;">' . mb_strimwidth($client['cus_mobile_phone'], 0, 15) . '</td>
            <td style="padding:5px;font-size:12px;width:50px;" align="right">' . number_format($client['cash_installments_daily']) . '</td>
            <td style="padding:5px;font-size:12px;width:80px;" align="right">' . number_format($chkoverduetodateSelect) . '</td>
            <td style="padding:5px;font-size:12px;width:70px;" align="right">' . number_format(($client['cash_principle'] + $client['cash_interest']) - $client['paysum_pay_amount']) . '</td>
        ...
        $content .= '
          <tr style="border: 1px solid black;white-space: nowrap;text-overflow: ellipsis;">
            <td style="padding:5px;font-size:12px;text-align:center;width:45px;">' . $i . '</td>
            <td style="padding:5px;font-size:12px;width:95px;">' . $client['contract_number'] . '</td>
            <td style="padding:5px;font-size:12px;width:150px;">' . $client['setpre_name'] . $client['cus_firstname'] . ' ' . $client['cus_lastname'] . '</td>
            <td style="padding:5px;font-size:12px;width:60px;">' . $client['cus_nickname'] . '</td>
...
$content .= '<tr style="border: 1px solid black;background-color: #D3D3D3;">
                <td style="padding:5px;font-size:13px;" colspan="5">รวม ' . $ii . ' คน</td>
                <td style="padding:5px;font-size:12px;" align="right">' . number_format($sumdaily) . '</td>
                <td style="padding:5px;font-size:12px;" align="right">' . number_format($sumoverdue) . '</td>
                <td style="padding:5px;font-size:12px;" align="right">' . number_format($sumtotaloverdue) . '</td>
                <td style="padding:5px;font-size:12px;"></td>
            </tr>';
$content .= '</table>
        ';
...
// $mpdf->SetHTMLHeader('{PAGENO}/{nbpg}');
